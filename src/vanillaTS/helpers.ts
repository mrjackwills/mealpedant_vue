...
export const formatCategoryName = (categoryName: string): string => categoryName.
	split(' ').
...
export const secondsToText = (s: number, short = true): string => {
	...
	if (day > 0) result += short ? `${day}d, ` : `${day} day${day === 1 ? '' : 's'}, `;
	if (hour > 0)result += short ? `${hour}h, ` : `${hour} hour${hour === 1 ? '' : 's'}, `;
...
export const todayDateString = (): string => new Date(Date.now() - new Date().getTimezoneOffset() * 60000).toISOString().
	slice(0, 10);

// Genesis date as string yyyy-mm-dd
export const genesisDateString = (): string => new Date(genesisDate).toISOString().
	slice(0, 10);
