...
const uncompress_meals = (input: c_MealInfo): MealInfo | undefined => {
	...
	try {
		const meal_descriptions = new Map<number, string>(
			Object.entries(input.d).map(([key, value]) => [Number(key), value])
		);

		const meal_categories = new Map<number, string>(
			Object.entries(input.c).map(([key, value]) => [Number(key), value])
		);
