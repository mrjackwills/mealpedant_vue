...
type ErrorData = { response: string }
...
type TypedMethod<This, Args extends unknown[], Return> = (
	this: This,
	...args: Args
) => Promise<Return>

function isAuthenticated<This, Args extends unknown[], Return> (
	target: TypedMethod<This, Args, Return>,
	_context: ClassMethodDecoratorContext<This, TypedMethod<This, Args, Return>>,
) {
...
function isNotAuthenticated<This, Args extends unknown[], Return> (
	target: TypedMethod<This, Args, Return>,
	_context: ClassMethodDecoratorContext<This, TypedMethod<This, Args, Return>>,
) {
	return async function (this: This, ...args: Args): Promise<Return | undefined> {
...
function isAdmin<This, Args extends unknown[], Return> (
	target: TypedMethod<This, Args, Return>,
	_context: ClassMethodDecoratorContext<This, TypedMethod<This, Args, Return>>,
) {
	return async function (this: This, ...args: Args): Promise<Return | undefined> {
		const isPrivileged = userModule().authenticated && userModule().admin
		if (isPrivileged) {
			return await target.call(this, ...args)
		}
		snackError({ message: 'invalid authentication' })
...
function wrap<This, Args extends unknown[], Return> (
	target: TypedMethod<This, Args, Return>,
	_context: ClassMethodDecoratorContext<This, TypedMethod<This, Args, Return>>,
) {
	return async function (this: This, ...args: Args): Promise<Return | undefined> {
		try {
			...
			const e = error as AxiosError<ErrorData>

			if (e.message === 'offline') {
				const store = browserModule()
				if (store.online) {
				...
				user.clear_email_admin()
				user.clientSideSignout()
				snackError({ message: 'you have been signed out' })
			} else {
				const message = e.response?.data?.response ?? 'unable to access server'
				snackError({ message })
			}
			return undefined as Return
		}
...
class Incognito {
	...
	@wrap
	@isNotAuthenticated
	async verify_get (verifyId: string): types.PUB {
		await baseAxios.get(`${this.#url}/verify/${verifyId}`)
		return true
	}

	@wrap
	async online_get (): types.PUB {
	...
	@wrap
	@isNotAuthenticated
	async register_post (registerObject: types.TRegisterUser): Promise<types.u<string>> {
	...
	@wrap
	@isNotAuthenticated
	async reset_get (hexId: string): types.PUB {
	...
	@wrap
	@isNotAuthenticated
	async reset_patch ({ resetId, password, token }: types.TPasswordPatch): types.PUB {
	...
	@wrap
	async meals_get (): Promise<types.u<types.c_MealInfo>> {
...
class AuthenticatedUser {
	...
	@wrap
	@isAuthenticated
	async password_patch (passwordObject: types.TPasswordChange): types.PUB {
	...
	@wrap
	@isAuthenticated
	async twoFA_delete (authObject: types.TAuthObject): types.PUB {
	...
	@wrap
	@isAuthenticated
	async twoFA_put (authObject: types.TAuthObject): types.PUB {
	...
	@wrap
	@isAuthenticated
	async setupTwoFA_get (): Promise<types.u<string>> {
	...
	@wrap
	@isAuthenticated
	async setupTwoFA_patch (body: types.TTFASetupPatch): types.PUB {
	...
	@wrap
	@isAuthenticated
	async setupTwoFA_post (token: types.TToken): types.PUB {
...
class AuthenticatedFood {
	...
	@wrap
	@isAuthenticated
	async all_get (): Promise<types.u<types.c_MealInfo>> {
	...
	@wrap
	@isAuthenticated
	async mealhash_get (): Promise<types.u<string>> {
		const response = await baseAxios.get(`${this.#url}/hash`)
		return response.data.response
	}
}
...
class Admin {
	...
	@wrap
	@isAdmin
	async backup_delete (file_name: string): types.PUB {
	...
	@wrap
	@isAdmin
	async backup_post (with_photos: boolean): types.PUB {
	...
	@wrap
	@isAdmin
	async email_post (emailData: types.TSendEmail): types.PUB {
	...
	@wrap
	@isAdmin
	async limit_delete (data: types.TUserLimit): types.PUB {
	...
	@wrap
	@isAdmin
	async session_get (email: string): Promise<types.u<Array<types.TAdminSession>>> {
	...
	@wrap
	@isAdmin
	async user_patch (userData: types.TAdminPatch): types.PUB {
...
class AdminMeal {
	...
	@wrap
	@isAdmin
	async meal_delete (data: types.TMealDelete): types.PUB {
	...
	@wrap
	@isAdmin
	async meal_patch (mealObject: types.TMealPatch): types.PUB {
	...
	@wrap
	@isAdmin
	async meal_post (meal: types.TMealDatePerson): types.PUB {
...
export class AdminPhoto {
	...
	@wrap
	@isAdmin
	async photo_delete (deleteData: types.TPhotoLong): types.PUB {
...
class DownloadPhoto {
	@wrap
	@isAuthenticated
	async photo_get (url: string): Promise<types.u<ArrayBuffer>> {
